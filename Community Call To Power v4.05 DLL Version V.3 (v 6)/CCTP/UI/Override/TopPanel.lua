...
ContextPtr:SetUpdate(function()
	...
	CurrentClock = os.date("[COLOR_POSITIVE_TEXT]%H:%M[/COLOR]");
	
	local strHiddenClockStr = "[ICON_PLUS]";

	if ClockBlue then

		local tElapsed = CurrentClock - StartTime;
		
		if (tElapsed > 5) then
			CurrentClock = os.date("[COLOR_WARNING_TEXT]%H:%M[/COLOR]");
		end
	...
	if (HideClock == true) then
		...
		Controls.ClockHidden:SetHide(false);
		Controls.ClockHidden:SetText(strHiddenClockStr);
		Controls.Clock:SetHide(true);
	
	else
	
		Controls.ClockHidden:SetHide(true);
		Controls.Clock:SetHide(false);
		Controls.Clock:SetText(CurrentClock);
...
function OnClockClicked()
	...
	if (ClockGreen == true) then
		print(" Colour Warning ON ");
		ClockGreen = false;
	else
		print(" Colour Warning OFF ");
		ClockGreen = true;
...
Controls.Clock:RegisterCallback( Mouse.eLClick, OnClockClicked );
Controls.Clock:RegisterCallback( Mouse.eRClick, OnClockRClicked );
Controls.ClockHidden:RegisterCallback( Mouse.eRClick, OnClockRClicked );
...
function UpdateData()
	...
	if( iPlayerID >= 0 ) then
		...
		if (pPlayer:GetNumCities() > 0) then
			...
			local strOrderStr;
			local iOrder = 0
			local iNumOrderCities = pPlayer:GetNumCities();
			for pCity in pPlayer:Cities() do
				local iCityOrder = GetCityOrder(pCity);
				iOrder = iOrder + iCityOrder;
			end	
			iOrder = (iOrder / iNumOrderCities);
			if iOrder < 0 then
				strOrderStr = string.format("[ICON_ORDER][COLOR:255:77:105:255]%i[/COLOR]", iOrder);
			else
				strOrderStr = string.format("[ICON_ORDER][COLOR:255:77:105:255]+%i[/COLOR]", iOrder);
			end
			Controls.OrderString:SetText(strOrderStr);
...
function DoInitTooltips()
	...
	Controls.OrderString:SetToolTipCallback( OrderTipHandler );
...
function HealthTipHandler( control )
	...
	local iCivHealthIndex, iTechModifier, iPolicyModifier, iTradeModifier, iUnHappyPenalty, iDifficultyPenalty = pPlayer:GetCivHealthIndex()
...
--end addition
--Order 
function OrderTipHandler( control )
	local iCityTotal = 0
	local strText = "";
	...
	local pPlayer = Players[Game.GetActivePlayer()];
		for pCity in pPlayer:Cities() do
			...
			iCityTotal = iCityTotal + iCityOrder;
			print("iCityTotal", iCityTotal);
		end	
	local g_OrderIndex, iOrderTechModifier, iOrderPolicyModifier, iUnHappyOrderPenalty, iDifficultyPenalty = pPlayer:GetCivOrderIndex()
	
	strText = strText .. Locale.ConvertTextKey("TXT_KEY_ORDER_OVERALL") .. ConvertOrderTextColor(iCityTotal);
	strText = strText .. "[NEWLINE]";

	--Policy modifier
	if (iOrderPolicyModifier ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(iOrderPolicyModifier) .. Locale.ConvertTextKey("TXT_KEY_TP_ORDER_POLICY");
	end

	--Tech modifier
	if (iOrderTechModifier ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(iOrderTechModifier) .. Locale.ConvertTextKey("TXT_KEY_TP_ORDER_TECH");
	end

	--Happiness modifier
	if (iUnHappyOrderPenalty ~= 0) then
		strText = strText .. "[NEWLINE]";
		if iUnHappyPenalty < 0 then
			strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(iUnHappyOrderPenalty) .. Locale.ConvertTextKey("TXT_KEY_TP_ORDER_UNHAPPY");
		else
			strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(iUnHappyOrderPenalty) .. Locale.ConvertTextKey("TXT_KEY_TP_ORDER_GOLDENAGE");
		end
	end
	...
	if (iDifficultyPenalty < 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(iDifficultyPenalty) .. Locale.ConvertTextKey("TXT_KEY_TP_ORDER_DIFFICULTY");
	end
...
function ConvertOrderTextColor(iValue)
	local strValue;
	if (iValue < 0) then
		strValue = Locale.ConvertTextKey("TXT_KEY_TP_ORDER_NEGATIVE", iValue);
	else
		strValue = Locale.ConvertTextKey("TXT_KEY_TP_ORDER_POSITIVE", iValue);
	end
	return strValue;
end
--end addition
