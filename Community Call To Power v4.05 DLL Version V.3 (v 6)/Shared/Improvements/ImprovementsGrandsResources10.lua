print("---ImprovementsGrandsResources10---");
include( "SaveUtils" ); MY_MOD_NAME = "ImprovementsGrandsResources10";
...
function onImprovementCreated(iHexX, iHexY)
	...
	if (pPlot~= nil) then
		...
		if(improvementType == GameInfoTypes.IMPROVEMENT_STATION) then
			...
			if(isPillaged) then
				if( aStation[plotID]~=nil and aStation[plotID].PlayerID ) then
					--need to take back Manpower resource
					DisconnectManpower( plotID, playerID );
				end
				print("Station Pillaged at plot ID:", plotID);
			else
				--we don't want to overwrite data if registered mine already exists on site
				if(aStation[plotID]==nil) then
					print("Station Built or Repaired", plotID);
...
function onImprovementDestroyed(iHexX, iHexY)
	...
	if(aStation[plotID]) then
		--local improvementType = pPlot:GetImprovementType();
		--if(improvementType == GameInfoTypes.IMPROVEMENT_STATION) then

		-- there used to be Station here - remove it
		print("onImprovementDestroyed: StationDestroyed");
		DisconnectManpower( plotID, aStation[plotID].PlayerID );
	end
...
function onPlotOwnershipChange(iHexX, iHexY)
	...
	if(aStation[plotID]) then
		print("onPlotOwnershipChange: Station Ownership hac changed");
		DisconnectManpower( plotID, aStation[plotID].PlayerID );
		local pPlot = Map.GetPlot(ToGridFromHex(iHexX, iHexY));
		if(pPlot) then
			...
			print("ERROR in StationControl: Map.GetPlot returned nil");
		end
	end
...
function ConnectManpower( plotID, playerID )
	...
	aStation[plotID] = {};
	aStation[plotID].PlayerID = playerID;
...
function DisconnectManpower( plotID, playerID )
	...
	--aStation[plotID].PlayerID = false;
	aStation[plotID] = nil;
	updateSaveData(plotID);
...
function updateSaveData(plotID)
	...
	save( pPlayer, plotID, aStation[plotID] );
end
