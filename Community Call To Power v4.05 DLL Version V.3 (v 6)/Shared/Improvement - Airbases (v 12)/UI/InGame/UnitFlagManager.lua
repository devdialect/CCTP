...
local g_UnitFlagClass = 
{
    ...
    Initialize = function( o, playerID, unitID, fogState, invisible )
		...
		o.m_IsTrade = isBNW and pUnit:IsTrade();
...
function OnHexFogEvent( hexPos, fogState, bWholeMap )
    ...
    then
        ...
        if( plot ~= nil ) then
            ...
            local unitCount = isBNW and plot:GetNumLayerUnits() or plot:GetNumUnits();	-- Get all layers, so we update the trade units as well
            
            for i = 0, unitCount - 1, 1
            do
                local unit = isBNW and plot:GetLayerUnit( i ) or plot:GetUnit( i );	-- Get all layers, so we update the trade units as well
                if( unit ~= nil ) 
                then
...
function GetAircraftText(pUnit)
	...
	if (damage ~= 0) then
		local healthTimes100 =  math.floor(100 * (1.0 - (damage / MaxDamage)) + 0.5);
