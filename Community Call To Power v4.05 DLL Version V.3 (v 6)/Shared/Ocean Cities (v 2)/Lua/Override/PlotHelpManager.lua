...
--OCEAN CITY MOD (Workaround for compatiblity with Health and Plague Mod)
include( "OceanCities_UI.lua" );

MapModData.g_Properties	= MapModData.g_Properties or {}
local g_Properties		= MapModData.g_Properties;
local m_OceanTempPlots;
--END
...
function Level1Tip()
	...
	if (plot and plot:IsRevealed(iActiveTeam, bIsDebug)) then
		...
		--OCEAN CITY MOD
		local tempData;
		if ((m_OceanTempPlots ~= nil) and (#m_OceanTempPlots > 0)) then
			if (m_OceanTempPlots[1].x == m_iCurrentX) then
				if (m_OceanTempPlots[1].y == m_iCurrentY) then
					tempData = m_OceanTempPlots[1]
				end
			end
		end
		--END
		...
		local strResource = "";
		if tempData then
			strResource = GetOCMODResourceString(plot, true, tempData);
		else
			strResource = GetResourceString(plot, true);
		end
		--END
		...
		-- Improvement, route
		--OCEAN CITY MOD
		local strImprovement = "";
		if tempData then
			strImprovement = GetOCMODImprovementString(plot, tempData);
		else
			strImprovement = GetImprovementString(plot);
		end
		--END
		...
		local natureStr = "";
		if tempData then
			natureStr = GetOCMODNatureString(plot, tempData);
		else
			natureStr = GetNatureString(plot);
		end
		--END
...
function DoUpdateXY( hexX, hexY )
	...
	if (plot ~= nil) then
		--OCEAN CITY MOD
		m_OceanTempPlots = g_Properties["g_OceanCitiesTempPlots"]
		m_iCurrentX = hexX;
		m_iCurrentY = hexY;
	end
